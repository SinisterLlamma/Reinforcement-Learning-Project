...
#SBATCH --output=/home2/eshaan.sharma/Reinforcement-Learning-Project/slurm_logs/deeptopj_%A_%a.out
#SBATCH --error=/home2/eshaan.sharma/Reinforcement-Learning-Project/slurm_logs/deeptopj_%A_%a.err
set -euo pipefail

# DeepTOP-J (Joint-Actor DeepTOP) full paper-config benchmark.
#
...
# DeepTOP-J actor architecture (overridable from submission env).
...
echo "Actor:   hidden=$ACTOR_HIDDEN heads=$ACTOR_HEADS layers=$ACTOR_LAYERS"
echo "M:       $M"
...
for SEED in "${SEEDS[@]}"; do
    LOG_CSV="$OUT_DIR/${ENV_NAME}_N${NB_ARMS}_V${BUDGET}_deeptopj_seed${SEED}.csv"
    echo "----"
