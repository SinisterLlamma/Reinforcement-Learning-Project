...
if __name__ == '__main__':
    ...
    parser.add_argument('--rate', default=0.001, type=float, help='critic learning rate')
    parser.add_argument('--prate', default=0.0001, type=float, help='actor learning rate')
    ...
    parser.add_argument('--M', default=1.0, type=float,
                        help='lambda sampling range bound; uniform(-M, M)')
    ...
    # Per-arm metadata: activation probability p_i (== q_i here).
    arm_metadata = np.asarray(p, dtype=np.float32).reshape(-1, 1)
